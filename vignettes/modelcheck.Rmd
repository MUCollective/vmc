...
Let’s fit a simple model on `datasets::mtcars` with four predictors:
...
model %>% 
  mcplot() +
  mc_distribution("predictive", newdata = "obs") +
  mc_model_geom_line(stat = "density", alpha = .1) +
  mc_obs_geom_line(stat = "density") +
  mc_comp_layout("sup") +
...
You can also show different draws by [HOPs](https://medium.com/hci-design-at-uw/hypothetical-outcomes-plots-experiencing-the-uncertain-b9ea60d7c740).

```{r}
model %>% 
  mcplot() +
  mc_model_geom_auto(draw = "hops") +
  mc_cond_var(x_var = vars(disp))
```

And applying HOPs to uncertainty representations...
...
model %>%
  ...
  mc_model_eye() +
  mc_obs_eye() +
  mc_comp_layout("nestjux") +
  mc_cond_var(x_var = vars(carb))
```
...
Residual plot with lineribbon:

```{r}
model %>%
  mcplot() + 
  mc_model_lineribbon() +
  mc_comp_layout("exp", exp_op = "residual") +
  mc_cond_var(x_var = vars(disp)) +
...
model %>%
  mcplot() + 
  mc_model_lineribbon() +
  mc_comp_layout("exp", exp_op = std_res_func) +
  mc_cond_var(x_var = vars(disp)) +
  mc_gglayer(geom_hline(yintercept = 0))
```
