...
To control how the distribution is drawn from model, you can add a `mc_distribution()` to `mcplot()`. Here, we are drawing the posterior distribution of `mu` from the Gaussian model we defined earlier.
...
You may find the distribution of `mpg` margining on `disp` is a little hard to see. You can use `mc_model_lineribbon()` to use line + ribbon plot to show the uncertainty of model.
