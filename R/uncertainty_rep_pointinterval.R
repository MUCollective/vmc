...
uncertainty_rep_pointinterval = function(..., n_sample = NA, draw = "collapse", group_on = NULL) {
  function(samples, row_vars, col_vars, labels, axis_type, model_color, is_animation, y_var) {
    ...
    if (is.function(draw)) {
      ...
      p = ggdist::stat_pointinterval(data = agg_sample,
                                     ggplot2::aes(y = y_agg,
                                                  color = !!model_color),
                                     ...)
      ...
      p = ggdist::stat_pointinterval(data = samples,
                                  ggplot2::aes(y = !!y_var, group = !!group_on,
                                               color = !!model_color),
                                  ...)
    } else if (draw == "hops") {
