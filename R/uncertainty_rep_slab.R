...
uncertainty_rep_slab = function(..., n_sample = NA, draw = "collapse", group_on = NULL) {
  function(samples, row_vars, col_vars, labels, axis_type, model_color, is_animation, y_var) {
    ...
    if (is.function(draw)) {
      ...
      return(c(ggdist::stat_slab(data = samples %>%
                                ...
                                mapping = ggplot2::aes(y = !!y_var, group = !!draw_col,
                                                      color = !!model_color, fill = !!model_color),
                                         ...),
